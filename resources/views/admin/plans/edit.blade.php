    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ $updateUrl }}" class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <input type="number" name="included_credits" class="form-control" value="{{ old('included_credits', $plan->included_credits) }}" required>
            </div>
            <div class="col-md-4">
                <label class="form-label">Overage Price (per 1K tokens)</label>
                <input type="number" step="0.01" name="overage_price_per_1k_tokens" class="form-control" value="{{ old('overage_price_per_1k_tokens', $plan->overage_price_per_1k_tokens) }}">
            </div>
            <div class="col-md-6">
                <label class="form-label">Max Requests / Day</label>
                <input type="number" name="max_requests_per_day" class="form-control" value="{{ old('max_requests_per_day', $plan->max_requests_per_day) }}">
            </div>
            <div class="col-md-6">
                <label class="form-label">Max Tokens / Day</label>
                <input type="number" name="max_tokens_per_day" class="form-control" value="{{ old('max_tokens_per_day', $plan->max_tokens_per_day) }}">
            </div>
            <div class="col-12">
                <label class="form-label">Model Multipliers (JSON)</label>
                <textarea name="model_multipliers" class="form-control" rows="4" placeholder='{"llama-3.3-70b-versatile": 1.2}'>{{ old('model_multipliers', json_encode($plan->model_multipliers ?? [], JSON_PRETTY_PRINT)) }}</textarea>
            </div>
            <div class="col-12">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="overageEnabled" name="overage_enabled" value="1" @checked(old('overage_enabled', $plan->overage_enabled))>
